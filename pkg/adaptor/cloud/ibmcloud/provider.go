...
func (p *ibmcloudVPCProvider) updateInstanceProfileSpecList() error {
	...
	for _, profileType := range instanceProfiles {
		...
		instanceProfileSpecList = append(instanceProfileSpecList, cloud.InstanceTypeSpec{InstanceType: profileType, VCPUs: vcpus, Memory: memory, Arch: arch})
	}
...
func (p *ibmcloudVPCProvider) selectImage(ctx context.Context, spec cloud.InstanceTypeSpec, selectedInstanceProfile string) (string, error) {
